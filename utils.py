'''Some helper functions:
    - ToRGB: convert image to RGB.
    - Log: log file.
    - summary: summary parameter number and operation number of network.
'''
import os
import sys
import time
...
def summary(model, input_size, batch_size=-1, device="cuda"):
    ...
    print("================================================================")
    print("W/O cls params num: {:,} ({:.2f} M)".format(wo_cls_params, 1. * wo_cls_params / (1000 ** 2.)))
    print("Total params num: {:,} ({:.2f} M)".format(total_params, 1. * total_params / (1000 ** 2.)))
    print("Trainable params num: {:,} ({:.2f} M)".format(trainable_params, 1. * trainable_params / (1000 ** 2.)))
...
def get_flops(model, input_size, device="cuda"):
    ...
    print('W/O cls FLOPs: %.2fG' % ((total_flops - cls_flops) / (1000 ** 3.)))
    print('Number of FLOPs: %.2fG' % (total_flops / (1000 ** 3.)))
    print("----------------------------------------------------------------")
